...
class FaceDetection(object):
    def __init__(UserGUI):
        UserGUI.detector = dlib.get_frontal_face_detector()
        UserGUI.predictor = dlib.shape_predictor("shape_predictor_68_face_landmarks.dat")
        UserGUI.fa = face_utils.FaceAligner(UserGUI.predictor, desiredFaceWidth=256)

    def face_detect(UserGUI, frame):
        ...
        if len(rects)>0:
            status = True
            # shape = UserGUI.predictor(gray, rects[0])
            ...
            face_frame = UserGUI.fa.align(frame,gray,rects[0]) # align face
            
            grayf = cv2.cvtColor(face_frame, cv2.COLOR_BGR2GRAY)
            rectsf = UserGUI.detector(grayf, 0)
            
            if len(rectsf) >0:
                shape = UserGUI.predictor(grayf, rectsf[0])
                shape = face_utils.shape_to_np(shape)
                ...
                rshape = UserGUI.face_remap(shape)    
